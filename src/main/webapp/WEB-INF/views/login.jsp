...
<body>
	...
	<div id="myLogin">
		...
		<a id="test" href="${APP_PATH}/meetingMa">跳转测试</a>
	</div>
	<script type="text/javascript">
		//错误信息
		...
		$("#reg_btn").click(function () {
			...
			$.ajax({
				...
				success : function name(result) {
					//console.log(result)
					if(result.code == 200){
						show_validate_msg($("#reg_email"),"error","该邮箱用户已存在");
						return false;
					}else{
						alert("注册成功");
			...
			//后台校验
 			$.ajax({
				...
				success : function(result) {
					//console.log(result)
 					if(result.code == 200){
						show_validate_msg($("#log_email"),"error","邮箱或密码不正确");
						return false;
					}else if(result.code == 100){
						window.location.href="${APP_PATH}/PersonnelMa";						
					}
				}
			})
		})
